...
const renderProjectForm = (project = undefined) => {
    ...
    for(const c of usersDiv.children){
        if(c.children[8].id === `project-${project.id}`){
...
User.all.forEach(user=>{
             ...
             user.projects.forEach(p=>{
                 ...
                 if(e.target.textContent === "Edit"){
                     ...
                     const findOldProject = Project.all.find(x => x === p);
                     ...
                     User.all[oldUserIndex].projects = []
                     User.all[oldUserIndex].projects.push(Project.all[oldProjectIndex])
                     usersDiv.innerHTML=""
